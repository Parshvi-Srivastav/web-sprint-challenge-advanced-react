...
export default class AppClass extends React.Component {
    ...
    switch (direction) {
      case "left":
        this.setState((prevState) => ({
          currentIdx:
            prevState.currentIdx % 3 !== 0
              ? prevState.currentIdx - 1
              : prevState.currentIdx,
        }));
        if (
          this.state.currentIdx === 0 ||
          this.state.currentIdx === 3 ||
          this.state.currentIdx === 6
        ) {
      ...
      case "right":
        this.setState((prevState) => ({
          currentIdx:
            prevState.currentIdx % 3 !== 2
              ? prevState.currentIdx + 1
              : prevState.currentIdx,
        }));
        if (
          this.state.currentIdx === 2 ||
          this.state.currentIdx === 5 ||
          this.state.currentIdx === 8
        ) {
      ...
      case "down":
        this.setState((prevState) => ({
          currentIdx:
            prevState.currentIdx < 6
              ? prevState.currentIdx + 3
              : prevState.currentIdx,
        }));
        if (
          this.state.currentIdx === 6 ||
          this.state.currentIdx === 7 ||
          this.state.currentIdx === 8
        ) {
  ...
  onSubmit = (evt) => {
    ...
    axios
      ...
      .catch((err) => {
        this.setState({ errMessages: err.response.data.message });
      });
  };

  componentDidUpdate(prevProps, prevState) {
    if (
      this.state.currentIdx !== initialState.index &&
      this.state.currentIdx !== prevState.currentIdx
    ) {
      this.setState(
  ...
  render() {
    ...
    return (
      <div id="wrapper" className={className}>
        ...
        <div id="grid">
          {[0, 1, 2, 3, 4, 5, 6, 7, 8].map((idx) => (
            <div
              key={idx}
              className={`square${
                idx === this.state.currentIdx ? " active" : ""
              }`}
            >
        ...
        <div className="info">
          <h3 id="message">
            {this.state.errMessages || this.state.successMessage}
          </h3>
        </div>
        <div id="keypad">
